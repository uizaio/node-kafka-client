...
Consumer.prototype.flowingConsume = function flowingConsume(handler) {
  ...
  this.consumer.on("data", async (message) => {
    const { partition } = message;
    if (!this.flowingCommitQueue[partition]) this.flowingCommitQueue[partition] = [];
    this.flowingCommitQueue[partition].push({ message, done: false });
...
Consumer.prototype.flowingCommit = function flowingCommit(message) {
  const { partition } = message;
  const index = this.flowingCommitQueue[partition].findIndex((m) => m.message.offset === message.offset);
